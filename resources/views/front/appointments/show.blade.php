                            ...
                            <div class="row gy-4">
                                    ...
                                    <div class="schedule-info mt-3">
                                        <div class="info-row">
                                            ...
                                            <div>
                                                ...
                                                <span
                                                    class="value">{{ \Carbon\Carbon::parse($appointment->appointment_datetime)->format('l, d F Y') }}</span>
                                            </div>
                                        </div>
                                        <div class="info-row mt-3">
                                            ...
                                            <div>
                                                ...
                                                <span
                                                    class="value">{{ \Carbon\Carbon::parse($appointment->appointment_datetime)->format('h:i A') }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-12 mt-4">
                                    <div class="reason-box p-3 rounded">
                                        <h6 class="fw-bold mb-2"><i class="bi bi-info-circle me-2"></i>Reason for Visit:
                                        </h6>
                                        <p class="mb-0">{{ $appointment->specialty->name }} -
                                            {{ $appointment->service->name }}</p>
                                        <p class="mb-0">
                                            {{ $appointment->patient_note ?? 'No additional notes provided.' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>

                        {{-- الجزء المعدل من الـ Ticket Footer --}}
                        <div class="ticket-footer d-flex justify-content-between align-items-center p-4">
                            ...
                            <div class="actions d-flex gap-2 flex-wrap justify-content-center">
                                ...
                                @if ($appointment->status == 'confirmed')
                                    <a href="{{ route('front.chat.clinic', $appointment->id) }}" class="btn btn-chat"
                                        id="openClinicChat">
                                        <i class="bi bi-chat-dots-fill me-2"></i>Open Chat
                                    </a>
                                @endif
                                @if ($appointment->status !== 'cancelled')
                                    <form action="{{ route('front.appointments.update', $appointment->id) }}"
                                        method="POST"
                                        onsubmit="return confirm('Are you sure you want to cancel this clinic appointment?')">
                                        @csrf
                                        @method('PUT')
